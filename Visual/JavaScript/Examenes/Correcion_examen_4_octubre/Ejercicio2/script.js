...
function inicio() {
    ...
    for (let i = 0; i < imagenes.length; i++) {
        ...
        //importante flujo de información del panel superior a la tabla
...
function eliminarProductoCesta(objetoEliminar) {
    ...
    let fila = objetoEliminar.target.parentNode.parentNode;
    //obtener el nombre de la fruta que se va a eliminar del carro
    let nombreFrutaEliminar = fila.querySelector('td:nth-child(1)').textContent;
    alert("Se va a eliminar: " + nombreFrutaEliminar);
   

    //obtener el precio parcial
    let precioParcial = parseFloat(fila.querySelector('td:nth-child(4)').textContent.replace(' €', ''));
